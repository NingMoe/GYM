...
import com.gy.pj.sys.dao.SysCoachDao;
...
public class SysCoachServiceImpl implements SysCoachService {
	...
	@Override
	public PageObject<SysCoach> findPageObjects(String name, Integer pageCurrent) {
		// 1.验证参数合法性
		...
		// 3.3)执行当前数据的查询操作
		List<SysCoach> records = sysCoachDao.findPageObjects(name, startIndex, pageSize);
		// 4.对分页信息以及当前页记录进行封装
		// 4.1)构建PageObject对象
		return  new PageObject<SysCoach>(pageCurrent, records, rowCount, pageSize);
	}
