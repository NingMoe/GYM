            ...
            if (d.list.length > 0) {
                $.each(d.list, function (i, n) {
                    if (n.courseType != 1) {
                        ...
                        str += '<li class="pic-box pull-left '+ checkSuperscriptType(n) +'">' +
                            ' <div class="hd-pic">' +
                            '     <a href="###"  class="tts-course-state" data-studymode = "'+n.studyMode+'" data-cn="'+n.className+'" data-code="'+n.studentOrderCode+'" data-courseversion="'+n.courseVersion+'" data-coursestate="'+n.courseState+'"  cusid="'+n.courseVersion +'">'+
                            '     <img src="' + n.urlSmall + '" alt="">' +
                            '     <i class="state"></i>' +
                            '   </a>' +
                            '  </div>' +
    ...
    $.ajax({
       ...
       // async: false,
        success: function (data) {
            if (data && data.code == 1) {
                // 是TTS用户，进入TTS
                window.location.href = TTS_MYTTS_URL + "/user/myTTS?sessionId=" + encodeURI(data.obj) + "&date=" + (encodeURI(new Date()))+"&courseVersion="+courseVersion;
            } else {
                // 统一跳转到着陆页
...
//提交验证码
function submitValidCode(courseversion,studymode) {
    $('.error-lty span').html('');//清空错误信息
    //
    //获取用户输入的信息
